...
#ifndef OPENSSL_NO_QUIC
static void report_quic_crypto_buffer_exceeded(SSL *ssl, const char *context)
{
    SSL_CONN_CLOSE_INFO cc_info;

    if (ssl == NULL || !SSL_is_quic(ssl))
        return;
    ...
    BIO_printf(bio_err,
               "%s: QUIC transport error CRYPTO_BUFFER_EXCEEDED -- handshake data exceeded the peer's CRYPTO buffer window.\n"
               "       Consider increasing the QUIC CRYPTO buffer sizes on both peers or reducing the handshake payload size.\n",
               context);
}
#endif
...
static int quic_debug_mode = 0;
static int quic_packet_log_mode = 0;
static int quic_bench_quiet = 0;
static int quic_measure_only = 0;
static double quic_debug_wallclock_ms(void)
{
#ifdef _WIN32
...
static void quic_client_debug_log(const char *fmt, ...)
{
    va_list args;
    double elapsed_ms;

    if (!quic_debug_mode || bio_err == NULL)
        return;

    elapsed_ms = quic_debug_wallclock_ms();
    fprintf(stderr, "[DEBUG][%12.8f ms][CLIENT][STATE] ", elapsed_ms);
    va_start(args, fmt);
...
static void quic_client_info_cb(const SSL *ssl, int where, int ret)
{
    if (!quic_debug_mode)
        return;

    if ((where & SSL_CB_HANDSHAKE_START) != 0)
        quic_client_debug_log("Handshake start ssl=%p", (const void *)ssl);
    if ((where & SSL_CB_HANDSHAKE_DONE) != 0)
        quic_client_debug_log("Handshake done ssl=%p ret=%d", (const void *)ssl, ret);
}
...
typedef enum OPTION_choice {
    ...
    OPT_KEYSHARE,
    OPT_QUIC_DEBUG,
    OPT_QUIC_BENCH_QUIET,
    OPT_QUIC_PACKET_LOG,
    OPT_QUIC_MEASURE_ONLY,
    OPT_ENABLE_PHA,
...
const OPTIONS s_client_options[] = {
    ...
    {"handshaketime", OPT_HANDSHAKE_TIME, '-', "Print the handshake RTT in milliseconds"},
    {"keyshare", OPT_KEYSHARE, '-', "Print ClientKeyShare and ServerKeyShare sizes in bytes"},
    {"quic_debug", OPT_QUIC_DEBUG, '-', "Enable QUIC benchmark debug logging (timestamped client events)"},
    {"quic_bench_quiet", OPT_QUIC_BENCH_QUIET, '-', "Suppress all client output except Handshake-RTT"},
    {"quic_packet_log", OPT_QUIC_PACKET_LOG, '-', "Print QUIC packet transmission logs"},
    {"quic_measure_only", OPT_QUIC_MEASURE_ONLY, '-', "Disable certificate logging/verification to benchmark raw handshake time"},
...
/* --- keyshare printing support (enabled only with --keyshare) --- */
typedef struct keyshare_cb_ctx_st {
    size_t *client_len;
    size_t *server_len;
    int also_debug;
    void *debug_arg;
...
/* Wrapper around the built-in tlsext_cb which also parses KeyShare (0x0033) to get key sizes */
static void keyshare_tlsext_cb(SSL *s, int client_server, int type,
                               const unsigned char *data, size_t len, void *arg)
{
    KEYSHARE_CB_CTX *ctx = (KEYSHARE_CB_CTX *)arg;
    if (ctx != NULL && ctx->also_debug)
        tlsext_cb(s, client_server, type, data, len, ctx->debug_arg);

    if (type != TLSEXT_TYPE_key_share || data == NULL)
        return;

    /* Try to detect ServerHello format first: group(2) + key_exchange_len(2) + key_exchange */
    if (len >= 4) {
        size_t kxlen = ((size_t)data[2] << 8) | data[3];
        if (4 + kxlen <= len) {
            if (ctx && ctx->server_len && *ctx->server_len == 0)
                *ctx->server_len = kxlen;
            return;
        }
    }

    /* Otherwise, assume ClientHello format: client_shares_len(2) + entries[] */
    if (len >= 2) {
        size_t listlen = ((size_t)data[0] << 8) | data[1];
        if (2 + listlen <= len && listlen >= 4) {
            size_t off = 2;
            /* Use the first KeyShareEntry */
            if (off + 4 <= len) {
                size_t kxlen = ((size_t)data[off + 2] << 8) | data[off + 3];
                if (off + 4 + kxlen <= len) {
                    if (ctx && ctx->client_len && *ctx->client_len == 0)
                        *ctx->client_len = kxlen;
                }
            }
        }
    }
}
/* --- end keyshare printing support --- */



/* --- KeyShare ClientHello/ServerHello capture via msg callback --- */

typedef void (*SSL_MSG_CB)(int, int, int, const void *, size_t, SSL *, void *);

typedef struct keyshare_msg_ctx_st {
    size_t *client_len;
    size_t *server_len;
    int also_msg;
    SSL_MSG_CB orig_cb;
...
/* Safe readers */
static int get_u8(const unsigned char **pp, size_t *plen, unsigned int *out)
{
    if (*plen < 1) return 0;
    *out = **pp;
    *pp += 1;
...
static int get_u16(const unsigned char **pp, size_t *plen, unsigned int *out)
{
    if (*plen < 2) return 0;
    *out = (((unsigned int)(*pp)[0]) << 8) | (*pp)[1];
    *pp += 2;
    *plen -= 2;
    return 1;
}

static int get_u24(const unsigned char **pp, size_t *plen, unsigned int *out)
{
    if (*plen < 3) return 0;
    *out = (((unsigned int)(*pp)[0]) << 16) | (((unsigned int)(*pp)[1]) << 8) | (*pp)[2];
    *pp += 3;
    *plen -= 3;
    return 1;
}

static int skip_bytes(const unsigned char **pp, size_t *plen, size_t n)
{
    if (*plen < n) return 0;
    *pp += n;
    *plen -= n;
...
/* Parse ClientHello extensions and extract first key_share key_exchange length */
static size_t parse_ch_keyshare_len(const unsigned char *p, size_t len)
{
    unsigned int hs_type, hs_len, u, ext_total, ext_type, ext_len, list_len, keylen;
    const unsigned char *q = p;
    size_t rlen = len;

    if (rlen < 4) return 0;
    hs_type = q[0]; /* no advance yet */
    if (hs_type != 1 /* SSL3_MT_CLIENT_HELLO */) return 0;
    q += 1; rlen -= 1;
    if (!get_u24(&q, &rlen, &hs_len)) return 0;
    if (rlen < hs_len) return 0;

    /* body parsing */
    if (!skip_bytes(&q, &rlen, 2 /* legacy_version */)) return 0;
    if (!skip_bytes(&q, &rlen, 32 /* random */)) return 0;
    if (!get_u8(&q, &rlen, &u)) return 0; /* session id len */
    if (!skip_bytes(&q, &rlen, u)) return 0;
    if (!get_u16(&q, &rlen, &u)) return 0; /* cipher_suites len */
    if (!skip_bytes(&q, &rlen, u)) return 0;
    if (!get_u8(&q, &rlen, &u)) return 0; /* compression methods len */
    if (!skip_bytes(&q, &rlen, u)) return 0;
    if (!get_u16(&q, &rlen, &ext_total)) return 0;
    if (rlen < ext_total) return 0;

    /* extensions loop */
    while (ext_total >= 4) {
        ext_type = (((unsigned int)q[0]) << 8) | q[1];
        ext_len  = (((unsigned int)q[2]) << 8) | q[3];
        q += 4;
        ext_total -= 4;
        if (ext_len > ext_total) return 0;
        if (ext_type == 0x0033 /* key_share */) {
            const unsigned char *e = q;
            size_t elen = ext_len;
            if (!get_u16(&e, &elen, &list_len)) return 0;
            if (elen < list_len) return 0;
            /* First KeyShareEntry */
            if (!get_u16(&e, &elen, &u)) return 0; /* group */
            if (!get_u16(&e, &elen, &keylen)) return 0; /* key_exchange len */
...
/* Parse ServerHello extensions and extract key_share key_exchange length */
static size_t parse_sh_keyshare_len(const unsigned char *p, size_t len)
{
    unsigned int hs_type, hs_len, u, ext_total, ext_type, ext_len, keylen;
    const unsigned char *q = p;
    size_t rlen = len;

    if (rlen < 4) return 0;
    hs_type = q[0];
    if (hs_type != 2 /* SSL3_MT_SERVER_HELLO */) return 0;
    q += 1; rlen -= 1;
    if (!get_u24(&q, &rlen, &hs_len)) return 0;
    if (rlen < hs_len) return 0;
    ...
    if (!get_u8(&q, &rlen, &u)) return 0; /* session id len */
    if (!skip_bytes(&q, &rlen, u)) return 0;
    if (!skip_bytes(&q, &rlen, 2 /* cipher_suite */)) return 0;
    if (!skip_bytes(&q, &rlen, 1 /* legacy_compression_method */)) return 0;
    if (!get_u16(&q, &rlen, &ext_total)) return 0;
    if (rlen < ext_total) return 0;
    ...
    while (ext_total >= 4) {
        ...
        if (ext_type == 0x0033 /* key_share */) {
            ...
            size_t elen = ext_len;
            unsigned int group;
            if (!get_u16(&e, &elen, &group)) return 0;
            if (!get_u16(&e, &elen, &keylen)) return 0;
            return (size_t)keylen;
        }
        q += ext_len;
        ext_total -= ext_len;
...
static void keyshare_msg_cb(int write_p, int version, int content_type,
                            const void *buf, size_t len, SSL *ssl, void *arg)
{
    KEYSHARE_MSG_CB_CTX *ctx = (KEYSHARE_MSG_CB_CTX *)arg;

    /* Chain to the original callback if requested */
    if (ctx && ctx->also_msg && ctx->orig_cb != NULL)
        ctx->orig_cb(write_p, version, content_type, buf, len, ssl, ctx->orig_arg);
    ...
    if (write_p) {
        ...
        size_t k = parse_ch_keyshare_len((const unsigned char *)buf, len);
        if (k > 0 && ctx && ctx->client_len != NULL)
            *(ctx->client_len) = k;
    } else {
        /* Incoming -> ServerHello */
        size_t k = parse_sh_keyshare_len((const unsigned char *)buf, len);
        if (k > 0 && ctx && ctx->server_len != NULL)
            *(ctx->server_len) = k;
    }
}
/* --- end KeyShare msg-callback support --- */
int s_client_main(int argc, char **argv)
{
    ...
    int fallback_scsv = 0;
    int print_hs_time = 0; 
    int print_keyshare = 0;
    size_t keyshare_client_bytes = 0, keyshare_server_bytes = 0;
    OPTION_CHOICE o;
#ifndef OPENSSL_NO_DTLS
...
# endif
	...
	case OPT_HANDSHAKE_TIME:
        ...
        case OPT_QUIC_DEBUG:
            quic_debug_mode = 1;
            break;
        case OPT_QUIC_BENCH_QUIET:
            quic_bench_quiet = 1;
            break;
        case OPT_QUIC_PACKET_LOG:
            quic_packet_log_mode = 1;
            quic_bench_quiet = 1;
            break;
        case OPT_QUIC_MEASURE_ONLY:
            quic_measure_only = 1;
            quic_bench_quiet = 1;
            break;
        case OPT_EOF:
...
# endif
    ...
    if (quic_measure_only && !quic_debug_mode)
        quic_bench_quiet = 1;
    if (quic_bench_quiet) {
        c_quiet = 1;
        c_ign_eof = 1;
    ...
    app_suppress_verify_details = (!quic_debug_mode && quic_bench_quiet) ? 1 : 0;
    apps_set_suppress_connect_prints(quic_bench_quiet);
    if (quic_debug_mode) {
        OSSL_QUIC_set_debug_role("CLIENT");
        OSSL_QUIC_set_debug_mode(1);
        quic_client_debug_log("Debug logging enabled");
    }

    if (quic_packet_log_mode || quic_debug_mode)
    ...
    if (quic_measure_only) {
        SSL_CTX_set_verify(ctx, SSL_VERIFY_NONE, NULL);
        quic_client_debug_log("Measure-only mode enabled: certificate verification disabled");
    }
...
# endif
    ...
    if (con == NULL)
        goto end;
    if (quic_debug_mode)
        SSL_set_info_callback(con, quic_client_info_cb);
 ...
 re_start:
    ...
    if (c_debug) {
        ...
        /* Set up message callback: if --keyshare is enabled we install a wrapper
     * that also captures ClientHello/ServerHello KeyShare sizes while chaining
     * to any user-requested -msg/-trace callback. */
    {
        SSL_MSG_CB orig_cb = NULL;
        void *orig_arg = bio_c_msg ? bio_c_msg : bio_c_out;
    #ifndef OPENSSL_NO_SSL_TRACE
        if (c_msg == 2)
            orig_cb = SSL_trace;
        else
        ...
        if (print_keyshare) {
            static KEYSHARE_MSG_CB_CTX ks_msg_ctx;
            ks_msg_ctx.client_len = &keyshare_client_bytes;
            ks_msg_ctx.server_len = &keyshare_server_bytes;
            ks_msg_ctx.also_msg  = c_msg ? 1 : 0;
            ks_msg_ctx.orig_cb   = orig_cb;
            ks_msg_ctx.orig_arg  = orig_arg;
            SSL_set_msg_callback(con, keyshare_msg_cb);
            SSL_set_msg_callback_arg(con, &ks_msg_ctx);
        } else if (c_msg) {
    #ifndef OPENSSL_NO_SSL_TRACE
            if (c_msg == 2)
            ...
            SSL_set_msg_callback_arg(con, orig_arg);
        }
    }
    if (c_tlsextdebug || print_keyshare) {
        if (print_keyshare) {
            static struct keyshare_cb_ctx_st ks_ctx;
            ks_ctx.client_len = &keyshare_client_bytes;
            ks_ctx.server_len = &keyshare_server_bytes;
            ks_ctx.also_debug = c_tlsextdebug;
            ks_ctx.debug_arg = bio_c_out;
            SSL_set_tlsext_debug_callback(con, keyshare_tlsext_cb);
            SSL_set_tlsext_debug_arg(con, &ks_ctx);
        } else {
            ...
            if (in_init) {
                ...
                if (print_hs_time) {
                    uint64_t rtt_us = 0;
                        ...
                        if (SSL_get_handshake_rtt(con, &rtt_us) == 1) {
                            double rtt_ms = rtt_us / 1000.0;

                        BIO *hs_stream = quic_bench_quiet ? bio_err : bio_c_out;

                        BIO_printf(hs_stream, "Handshake-RTT: %.2f ms\n", rtt_ms);
                        }
                }

                if (print_keyshare) {
                    if (keyshare_client_bytes > 0)
                        BIO_printf(bio_err, "ClientKeyShare: %zu bytes\n", keyshare_client_bytes);
                    if (keyshare_server_bytes > 0)
                        BIO_printf(bio_err, "ServerKeyShare: %zu bytes\n", keyshare_server_bytes);
                }
                if (!quic_bench_quiet)
                ...
                if (quic_measure_only || quic_bench_quiet) {
                    ret = 0;
                    goto shut;
                }
            }
...
/* OPENSSL_SYS_MSDOS includes OPENSSL_SYS_WINDOWS */
 ...
 shut:
    ...
    if (con != NULL) {
#ifndef OPENSSL_NO_QUIC
        report_quic_crypto_buffer_exceeded(con, "s_client");
#endif
        if (prexit != 0)
            if (!quic_bench_quiet)
    ...
    BIO_free(bio_c_msg);
    bio_c_msg = NULL;
    apps_set_suppress_connect_prints(0);
    return ret;
}
