...
QUIC_PORT *ossl_quic_port_new(const QUIC_PORT_ARGS *args)
{
    ...
    port->unlimited_amplification = args->unlimited_amplification ? 1 : 0;
...
static int port_init(QUIC_PORT *port)
{
    ...
    port->tx_max_udp_payload_size = QUIC_MIN_INITIAL_DGRAM_LEN;
...
int ossl_quic_port_set_net_wbio(QUIC_PORT *port, BIO *net_wbio)
{
    ...
    if (net_wbio != NULL) {
        mtu = BIO_dgram_get_mtu(net_wbio);
        if (mtu >= QUIC_MIN_INITIAL_DGRAM_LEN) {
            new_mdpl = mtu;
            port->tx_max_udp_payload_size = new_mdpl;
        }
    }
    ...
    if (new_mdpl != 0) {
        OSSL_LIST_FOREACH(ch, ch, &port->channel_list)
            ossl_quic_channel_handle_local_mtu_update(ch, new_mdpl);
    }
                              ...
                              QUIC_CHANNEL **new_ch)
{
    ...
    if (port->unlimited_amplification) {
        OSSL_QUIC_DEBUG_LOG(stderr, "UNLIMITED",
                            "Bypassing anti-amplification for incoming connection");
    ...
    if (odcid->id_len != 0) {
         ...
         * See RFC 9000 s. 8.1
         */
        OSSL_QUIC_DEBUG_LOG(stderr, "RETRY",
                            "odcid.id_len=%d, marking channel as VALIDATED",
                            (int)odcid->id_len);
        ossl_quic_tx_packetiser_set_validated(ch->txp);
                                        ...
                                        const QUIC_CONN_ID *dcid)
{
    ...
    if (port->validate_addr == 1 && hdr.token == NULL) {
        OSSL_QUIC_DEBUG_LOG(stdout, "RETRY",
                            "No token, sending RETRY packet");
     ...
     * validate here
     */
    int token_validated = 0;
    if (hdr.token != NULL) {
        OSSL_QUIC_DEBUG_LOG(stderr, "RETRY",
                            "Token received, validating...");
        int token_valid = port_validate_token(&hdr, port, &e->peer,
                                               &odcid, &scid,
                                               &gen_new_token);
        OSSL_QUIC_DEBUG_LOG(stderr, "RETRY",
                            "Token validation result: %d", token_valid);
        if (token_valid == 0) {
            /*
             * RFC 9000 s 8.1.3
            ...
            if (port->validate_addr == 1) {
                 ...
                 */
                OSSL_QUIC_DEBUG_LOG(stderr, "RETRY",
                                    "Token invalid, sending another RETRY");
                port_send_retry(port, &e->peer, &hdr);
                goto undesirable;
            }
            ...
            /* Token is valid - client has been validated via RETRY */
            OSSL_QUIC_DEBUG_LOG(stderr, "RETRY",
    ...
    OSSL_QUIC_DEBUG_LOG(stderr, "RETRY",
                        "Before port_bind_channel, odcid.id_len=%d",
                        (int)odcid.id_len);
    ...
    OSSL_QUIC_DEBUG_LOG(stderr, "RETRY", "Channel created successfully");
