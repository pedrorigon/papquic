...
#define INIT_CRYPTO_RECV_BUF_LEN    65536
#define INIT_CRYPTO_SEND_BUF_LEN    65536
...
static void ch_update_tx_mdpl(QUIC_CHANNEL *ch);
...
static int ch_init(QUIC_CHANNEL *ch)
{
    ...
    ch->tx_max_udp_payload_size = qtx_args.mdpl;
    ...
    if (ch->port != NULL)
        ossl_quic_channel_handle_local_mtu_update(ch, ch->port->tx_max_udp_payload_size);
    ...
    txp_args.is_server              = ch->is_server ? 1 : 0;
    txp_args.tls                    = ch->tls;
    txp_args.qtx                    = ch->qtx;
...
static void ch_update_tx_mdpl(QUIC_CHANNEL *ch)
{
    ...
    effective = ch->tx_max_udp_payload_size;
    if (effective < QUIC_MIN_INITIAL_DGRAM_LEN)
        effective = QUIC_MIN_INITIAL_DGRAM_LEN;

    peer_limit = ch->rx_max_udp_payload_size > (uint64_t)SIZE_MAX
                     ? SIZE_MAX
                     : (size_t)ch->rx_max_udp_payload_size;

    if (peer_limit != 0 && peer_limit < effective)
        effective = peer_limit;

    if (effective < QUIC_MIN_INITIAL_DGRAM_LEN)
        effective = QUIC_MIN_INITIAL_DGRAM_LEN;

    if (ch->qtx != NULL)
        ossl_qtx_set_mdpl(ch->qtx, effective);
}

void ossl_quic_channel_handle_local_mtu_update(QUIC_CHANNEL *ch, size_t mdpl)
{
    if (ch == NULL)
        return;
    ...
    ch->tx_max_udp_payload_size = mdpl;
    ch_update_tx_mdpl(ch);
}
...
static void ch_rx_handle_packet(QUIC_CHANNEL *ch, int channel_only)
{
    ...
    case QUIC_PKT_TYPE_RETRY:
        OSSL_QUIC_DEBUG_LOG(stdout, "CLIENT", "Received RETRY packet");
        fflush(stdout);
        ...
        if (ch->doing_retry || ch->is_server) {
             ...
             */
            OSSL_QUIC_DEBUG_LOG(stdout, "CLIENT",
                                "Ignoring RETRY (already doing retry or is server)");
            fflush(stdout);
            return;
        }
        ...
        if (ch->have_received_enc_pkt) {
            OSSL_QUIC_DEBUG_LOG(stdout, "CLIENT",
                                "Ignoring RETRY (already received encrypted packet)");
            fflush(stdout);
            return;
        }
        ...
        if (ch->qrx_pkt->hdr->len <= QUIC_RETRY_INTEGRITY_TAG_LEN) {
            ...
            OSSL_QUIC_DEBUG_LOG(stdout, "CLIENT",
                                "Ignoring RETRY (zero-length token)");
            fflush(stdout);
            return;
        }
        ...
        if (!ossl_quic_validate_retry_integrity_tag(ch->port->engine->libctx,
            ...
            OSSL_QUIC_DEBUG_LOG(stdout, "CLIENT",
                                "Ignoring RETRY (invalid integrity tag)");
            fflush(stdout);
            return;
        }

        OSSL_QUIC_DEBUG_LOG(stdout, "CLIENT",
                            "RETRY packet valid, calling ch_retry");
        fflush(stdout);
        ...
        if (!ch_retry(ch, ch->qrx_pkt->hdr->data,
            ...
            OSSL_QUIC_DEBUG_LOG(stdout, "CLIENT", "ch_retry FAILED");
            fflush(stdout);
            ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_INTERNAL_ERROR,
                                                   0, "handling retry packet");
        } else {
            OSSL_QUIC_DEBUG_LOG(stdout, "CLIENT",
                                "ch_retry SUCCESS - will retransmit Initial with token");
            fflush(stdout);
            /*
             * Force immediate TX to send the Initial packet with token
             * without waiting for the next tick/PTO. This only affects
             * RETRY-based validation (QUIC_MODE=3) and doesn't impact
             * normal handshake flows (QUIC_MODE=0,1,2).
             */
            ch_tx(ch, NULL);
        }
        break;
                    ...
                    int drop_later_pn)
{
    ...
    /*
     * Schedule an immediate ACK-eliciting packet to trigger retransmission
     * of the Initial packet with the token. This avoids waiting for the PTO
     * timer and ensures the retry happens immediately.
     */
    ossl_quic_tx_packetiser_schedule_ack_eliciting(ch->txp, QUIC_PN_SPACE_INITIAL);
