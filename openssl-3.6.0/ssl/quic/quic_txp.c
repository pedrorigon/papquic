 ...
 *   - It normally may not contain anything other than an ACK frame. When
 *     handshake ACK padding is enabled we may append PADDING frames as well.
 */
...
static void txp_debug(OSSL_QUIC_TX_PACKETISER *txp, const char *component,
                      const char *fmt, ...)
{
    va_list args;
    char buffer[256];

    if (txp == NULL || !txp->args.is_server)
        return;
    ...
    OSSL_QUIC_DEBUG_LOG(stdout, component, "%s", buffer);
}

static unsigned char *txp_flatten_payload(const OSSL_QTX_IOVEC *iovec,
                                          size_t num_iovec,
                                          size_t *out_len)
{
    size_t total = 0, i;
    ...
    cursor = buf;
    for (i = 0; i < num_iovec; ++i) {
        if (iovec[i].buf_len == 0)
            continue;
        memcpy(cursor, iovec[i].buf, iovec[i].buf_len);
        cursor += iovec[i].buf_len;
    }

    return buf;
}

static const char *txp_amp_state(size_t credit)
{
    return (credit == SIZE_MAX) ? "UNLIMITED" : "3X";
}
...
static int tx_helper_expand_max_ppl(struct tx_helper *h, size_t new_max_ppl)
{
    struct txp_el *el;

    if (new_max_ppl <= h->max_ppl)
        return 1;

    el = &h->txp->el[h->enc_level];

    if (new_max_ppl > el->scratch_len) {
        unsigned char *scratch = OPENSSL_realloc(el->scratch, new_max_ppl);

        if (scratch == NULL)
            return 0;

        el->scratch = scratch;
        el->scratch_len = new_max_ppl;
    }

    h->max_ppl = new_max_ppl;
    return 1;
}
...
static int txp_pkt_commit(OSSL_QUIC_TX_PACKETISER *txp, struct txp_pkt *pkt,
                          ...
                          size_t credit_before, size_t credit_after);
...
void ossl_quic_tx_packetiser_set_validated(OSSL_QUIC_TX_PACKETISER *txp)
{
    if (txp->unvalidated_credit != SIZE_MAX) {
        size_t before = txp->unvalidated_credit;

        txp->unvalidated_credit = SIZE_MAX;
        txp_debug(txp, "CREDIT-STATE",
                  "Amplification window transition: SET_VALIDATED (%s -> UNLIMITED)",
                  txp_amp_state(before));
    }
    return;
}
                                                    ...
                                                    size_t credit)
{
    if (txp->unvalidated_credit != SIZE_MAX) {
        size_t before = txp->unvalidated_credit;
        size_t delta = credit * 3;

        if ((SIZE_MAX - before) > delta)
            txp->unvalidated_credit = before + delta;
        else
            txp->unvalidated_credit = SIZE_MAX - 1;
        txp_debug(txp, "CREDIT-ACCUMULATOR",
                  "Amplification window (3X): ADD_UNVALIDATED_CREDIT bytes=%zu | before=%zu | after=%zu",
                  delta, before, txp->unvalidated_credit);
    } else {
        txp_debug(txp, "CREDIT-ACCUMULATOR",
                  "Amplification window (UNLIMITED): ADD_UNVALIDATED_CREDIT ignored");
    }
    return;
}
                                                        ...
                                                        size_t credit)
{
    if (txp->unvalidated_credit != SIZE_MAX) {
        ...
        txp_debug(txp, "CREDIT-CONSUMER",
                  "Amplification window (3X): CONSUME_UNVALIDATED_CREDIT bytes=%zu | before=%zu | after=%zu",
                  credit, before, txp->unvalidated_credit);
    } else {
        txp_debug(txp, "CREDIT-CONSUMER",
                  "Amplification window (UNLIMITED): CONSUME_UNVALIDATED_CREDIT ignored");
    }
}
 ...
 * QUIC TX packetiser meets or exceeds the required credit value
 * (`req_credit`). If sufficient credit is available, the function returns 1
 ...
 *
 * @return 1 if the unvalidated credit is at least `req_credit`, 0 otherwise.
 */
int ossl_quic_tx_packetiser_check_unvalidated_credit(OSSL_QUIC_TX_PACKETISER *txp,
                                                     size_t req_credit)
{
    size_t available = txp->unvalidated_credit;
    int ok = (available == SIZE_MAX) || (available >= req_credit);

    txp_debug(txp, "CREDIT-CHECK",
              "Amplification window (%s): CHECK_UNVALIDATED_CREDIT need=%zu | available=%zu | result=%s",
              txp_amp_state(available), req_credit, available, ok ? "ALLOW" : "BLOCK");
    return ok;
}

size_t ossl_quic_tx_packetiser_get_unvalidated_credit(const OSSL_QUIC_TX_PACKETISER *txp)
{
    return txp == NULL ? SIZE_MAX : txp->unvalidated_credit;
}
...
{
    txp_debug(txp, "TOKEN",
              "SET_INITIAL_TOKEN requested_len=%zu", token_len);

    if (!txp_check_token_len(token_len, txp_get_mdpl(txp))) {
        txp_debug(txp, "TOKEN",
                  "SET_INITIAL_TOKEN rejected (too large for MDPL)");
        return 0;
    }
    ...
    txp_debug(txp, "TOKEN",
              "SET_INITIAL_TOKEN success stored_len=%zu",
              txp->initial_token_len);
    return 1;
}
                                     ...
                                     QUIC_TXP_STATUS *status)
{
    ...
    /*
     * Mode 4: Reactive RETRY detection disabled temporarily.
     * TODO: Implement full RETRY mechanism (send RETRY packet, validate token,
     * set unvalidated_credit = SIZE_MAX). For now, Mode 4 only uses ACK padding.
     */
    /*
    if (txp_is_stalled_by_credit(txp))
        return 1;
    */
    ...
    if (need_padding) {
        ...
        for (enc_level = QUIC_ENC_LEVEL_INITIAL;
            ...
            if (pkt[enc_level].h_valid && pkt[enc_level].h.bytes_appended > 0) {
                ...
                if (pkt[enc_level].geom.cmpl >= pkt[enc_level].geom.pkt_overhead) {
                    size_t new_max_ppl = pkt[enc_level].geom.cmpl
                        - pkt[enc_level].geom.pkt_overhead;

                    if (!tx_helper_expand_max_ppl(&pkt[enc_level].h, new_max_ppl))
                        goto out;

                    pkt[enc_level].geom.cmppl = pkt[enc_level].h.max_ppl;
                }
...
static int txp_get_archetype_data(OSSL_QUIC_TX_PACKETISER *txp,
                                  uint32_t enc_level,
                                  uint32_t archetype,
                                  ...
                                  struct txp_pkt_geom *geom)
{
    ...
    if (!txp_get_archetype_data(txp, enc_level, archetype, &geom->adata))
        return 0;
    ...
    if (enc_level == QUIC_ENC_LEVEL_INITIAL) {
        ...
        txp_debug(txp, "PACKET-BUILD",
                  "GENERATE Initial token_len=%zu pn=%" PRIu64,
                  txp->initial_token_len, txp->next_pn[pn_space]);
    } else {
        phdr->token     = NULL;
        phdr->token_len = 0;
    }
                                  ...
                                  uint32_t *conn_close_enc_level)
{
    ...
    if (!txp_get_archetype_data(txp, enc_level, archetype, &a))
        return 0;
                          ...
                          size_t credit_after)
{
    ...
    *txpim_pkt_reffed = 1;
    ...
    if (ossl_quic_packet_log_enabled()) {
        size_t payload_len = 0;
        unsigned char *payload_copy = txp_flatten_payload(txpkt.iovec,
                                                          txpkt.num_iovec,
                                                          &payload_len);
        long long credit_delta = 0;
        int have_credit_delta = 0;

        if (txp->args.is_server) {
            if (credit_before == SIZE_MAX || credit_after == SIZE_MAX)
                credit_delta = 0;
            else
                credit_delta = (long long)credit_after - (long long)credit_before;
            have_credit_delta = 1;
        }

        OSSL_QUIC_PACKET_LOG_EVENT event = {
            .stream = stderr,
            .is_server = txp->args.is_server,
            .is_send = 1,
            .enc_level = enc_level,
            .packet_type = ossl_quic_packet_type_name(pkt->phdr.type),
            .packet_number = txpkt.pn,
            .datagram_size = tpkt->ackm_pkt.num_bytes,
            .payload = payload_copy,
            .payload_len = payload_len,
            .have_credit_before = txp->args.is_server,
            .credit_before = credit_before,
            .credit_before_unlimited = (credit_before == SIZE_MAX),
            .have_credit_after = txp->args.is_server,
            .credit_after = credit_after,
            .credit_after_unlimited = (credit_after == SIZE_MAX),
            .client_credit_known = !txp->args.is_server,
            .client_credit_unlimited = (credit_after == SIZE_MAX),
            .have_credit_delta = have_credit_delta,
            .credit_delta = credit_delta,
            .ssl = txp->args.tls,
        };

        ossl_quic_packet_log(&event);
        OPENSSL_free(payload_copy);
    }
