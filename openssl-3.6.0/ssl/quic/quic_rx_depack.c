...
int ossl_quic_handle_frames(QUIC_CHANNEL *ch, OSSL_QRX_PKT *qpacket)
{
    ...
    int log_packet = ossl_quic_packet_log_enabled();
    size_t datagram_size = 0;
    size_t credit_before = 0;
    size_t credit_after = 0;
    int have_credit_snapshot = 0;
    ...
    if (log_packet) {
        datagram_size = qpacket->datagram_len;
        if (ch->txp != NULL) {
            credit_before = ossl_quic_tx_packetiser_get_unvalidated_credit(ch->txp);
            credit_after = credit_before;
            have_credit_snapshot = 1;
        }
    }
    ...
    if (enc_level == QUIC_ENC_LEVEL_HANDSHAKE) {
        const char *role = ch->is_server ? "SERVER" : "CLIENT";

        OSSL_QUIC_DEBUG_LOG(stderr, role,
                            "Received HANDSHAKE packet -> validating connection (removing 3x limit)");
        if (ch->txp != NULL)
    ...
    if (have_credit_snapshot && ch->txp != NULL)
        credit_after = ossl_quic_tx_packetiser_get_unvalidated_credit(ch->txp);

    if (log_packet) {
        long long credit_delta = 0;
        int have_credit_delta = 0;

        if (ch->is_server && have_credit_snapshot) {
        ...
        OSSL_QUIC_PACKET_LOG_EVENT event = {
            ...
            .is_send = 0,
            .enc_level = enc_level,
            .packet_type = ossl_quic_packet_type_name(qpacket->hdr->type),
            .packet_number = qpacket->pn,
            .datagram_size = datagram_size,
            .payload = qpacket->hdr->data,
            .payload_len = qpacket->hdr->len,
            .have_credit_before = ch->is_server && have_credit_snapshot,
            .credit_before = credit_before,
            .credit_before_unlimited = (have_credit_snapshot
                                        && credit_before == SIZE_MAX),
            .have_credit_after = ch->is_server && have_credit_snapshot,
            .credit_after = credit_after,
            .credit_after_unlimited = (have_credit_snapshot
                                       && credit_after == SIZE_MAX),
            .client_credit_known = !ch->is_server && have_credit_snapshot,
            .client_credit_unlimited = (have_credit_snapshot
                                        && credit_after == SIZE_MAX),
