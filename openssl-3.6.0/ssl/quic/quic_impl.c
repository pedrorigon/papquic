...
void OSSL_QUIC_set_debug_mode(int enabled)
{
    ossl_quic_debug_set_enabled(enabled != 0);
}

void OSSL_QUIC_set_debug_role(const char *role)
{
    ossl_quic_debug_set_role(role);
}

void OSSL_QUIC_set_packet_log_mode(int enabled)
{
    ossl_quic_packet_log_set_enabled(enabled != 0);
}
#endif
...
SSL *ossl_quic_new_listener(SSL_CTX *ctx, uint64_t flags)
{
    ...
    if ((flags & SSL_LISTENER_FLAG_UNLIMITED_AMPLIFICATION) != 0)
        port_args.unlimited_amplification = 1;
