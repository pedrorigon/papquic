...
WORK_STATE tls_post_process_client_hello(SSL_CONNECTION *s, WORK_STATE wst)
{
    ...
    ossl_quic_timing_server_client_hello_process_begin(s);
#endif
    ...
    if (wst == WORK_MORE_A) {
        ...
        wst = WORK_MORE_B;
#ifndef OPENSSL_NO_QUIC
        ossl_quic_timing_server_client_hello_processed(s);
#endif
    }
    if (wst == WORK_MORE_B) {
        if (!s->hit || SSL_CONNECTION_IS_TLS13(s)) {
...
#ifndef OPENSSL_NO_QUIC
    ossl_quic_timing_server_hello_ready(s);
#endif
...
#ifndef OPENSSL_NO_QUIC
    int measuring_cert = SSL_IS_QUIC_HANDSHAKE(s);
#endif
...
#ifndef OPENSSL_NO_QUIC
        if (measuring_cert)
            ossl_quic_timing_server_cert_build_abort(s);
#endif
        return CON_FUNC_ERROR;
    }
...
#ifndef OPENSSL_NO_QUIC
    if (measuring_cert)
        ossl_quic_timing_server_cert_build_begin(s);
#endif
...
#ifndef OPENSSL_NO_QUIC
    if (measuring_cert)
        ossl_quic_timing_server_cert_build_complete(s);
#endif
    return CON_FUNC_SUCCESS;
...
{
#ifndef OPENSSL_NO_QUIC
    ossl_quic_timing_server_handshake_build_begin(s);
#endif
