...
# ifndef OPENSSL_NO_QUIC
        measuring_keyshare = 1;
        ossl_quic_timing_client_keyshare_begin(s);
# endif
        key_share_key = ssl_generate_pkey_group(s, group_id);
        if (key_share_key == NULL) {
            /* SSLfatal() already called */
# ifndef OPENSSL_NO_QUIC
            if (measuring_keyshare)
                ossl_quic_timing_client_keyshare_abort(s);
# endif
            return 0;
        }
    }
...
# ifndef OPENSSL_NO_QUIC
    if (measuring_keyshare)
        ossl_quic_timing_client_keyshare_complete(s);
# endif
                             ...
                             size_t chainidx)
{
     ...
     */
#ifndef OPENSSL_NO_QUIC
    ossl_quic_timing_client_kex_begin(s);
#endif
    for (i = 0; i < s->s3.tmp.num_ks_pkey; i++) {
        if (s->s3.tmp.ks_group_id[i] == group_id) {
    ...
    if (valid_ks_id == 0) {
        ...
        SSLfatal(s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_BAD_KEY_SHARE);
#ifndef OPENSSL_NO_QUIC
        ossl_quic_timing_client_kex_abort(s);
#endif
        return 0;
    }
    ...
    s->s3.did_kex = 1;
#ifndef OPENSSL_NO_QUIC
    ossl_quic_timing_client_kex_complete(s);
#endif
#endif
