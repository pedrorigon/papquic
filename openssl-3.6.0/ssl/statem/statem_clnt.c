...
CON_FUNC_RETURN tls_construct_client_hello(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    ossl_quic_timing_client_hello_build_begin(s);
#endif
...
#ifndef OPENSSL_NO_QUIC
    ossl_quic_timing_client_hello_build_complete(s);
    ossl_quic_timing_client_hello_sent(s);
#endif
...
#ifndef OPENSSL_NO_QUIC
    ossl_quic_timing_client_processed_server_hello(s);
#endif
                                               ...
                                               WORK_STATE wst)
{
...
#ifndef OPENSSL_NO_QUIC
    ossl_quic_timing_client_cert_chain_begin(s);
#endif
    ...
    if (i <= 0 && s->verify_mode != SSL_VERIFY_NONE) {
        ...
        SSLfatal(s, ssl_x509err2alert(s->verify_result),
                 SSL_R_CERTIFICATE_VERIFY_FAILED);
        ossl_quic_timing_client_cert_chain_abort(s);
        return WORK_ERROR;
    }
    ERR_pop_to_mark();      /* but we keep s->verify_result */
...
#ifndef OPENSSL_NO_QUIC
    ossl_quic_timing_client_cert_chain_complete(s);
#endif
    return WORK_FINISHED_CONTINUE;
}
