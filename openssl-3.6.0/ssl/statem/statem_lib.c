...
CON_FUNC_RETURN tls_construct_cert_verify(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    int measuring_cv = SSL_IS_QUIC_HANDSHAKE(s) && s->server;
#endif
...
#ifndef OPENSSL_NO_QUIC
    if (measuring_cv)
        ossl_quic_timing_server_cert_verify_begin(s);
#endif
...
#ifndef OPENSSL_NO_QUIC
    if (measuring_cv)
        ossl_quic_timing_server_cert_verify_complete(s);
#endif
...
#ifndef OPENSSL_NO_QUIC
    if (measuring_cv)
        ossl_quic_timing_server_cert_verify_abort(s);
#endif
...
MSG_PROCESS_RETURN tls_process_cert_verify(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    SSL_CTX *sctx = SSL_CONNECTION_GET_CTX(s);
#ifndef OPENSSL_NO_QUIC
    int measuring_cv_check = SSL_IS_QUIC_HANDSHAKE(s) && !s->server;
#endif
...
#ifndef OPENSSL_NO_QUIC
    if (measuring_cv_check)
        ossl_quic_timing_client_cert_verify_check_begin(s);
#endif
 ...
 err:
    ...
    if (measuring_cv_check) {
        if (ret == MSG_PROCESS_ERROR)
            ossl_quic_timing_client_cert_verify_check_abort(s);
        else
            ossl_quic_timing_client_cert_verify_check_complete(s);
    }
#endif
...
#ifndef OPENSSL_NO_QUIC
    if (s->server)
        ossl_quic_timing_server_handshake_build_complete(s);
#endif
    return CON_FUNC_SUCCESS;
}
