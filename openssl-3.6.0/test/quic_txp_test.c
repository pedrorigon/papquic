...
static int helper_init(struct helper *h)
{
    ...
    /* Amplification is now hardcoded to 3x in quic_txp.c */
...
/* 19 & 20. Handshake ACK padding behaviour */
static int schedule_handshake_ack(struct helper *h)
{
    size_t i;
    OSSL_ACKM_RX_PKT rx_pkt = {0};
    ...
    for (i = 0; i < 5; ++i) {
        rx_pkt.pkt_num          = i;
        rx_pkt.time             = fake_now(NULL);
        rx_pkt.pkt_space        = QUIC_PN_SPACE_HANDSHAKE;
        rx_pkt.is_ack_eliciting = 1;
...
static int check_handshake_ack_packet(struct helper *h)
{
    if (!TEST_ptr(h->qrx_pkt) || !TEST_ptr(h->qrx_pkt->hdr))
        return 0;

    return TEST_int_eq((int)h->qrx_pkt->hdr->type, QUIC_PKT_TYPE_HANDSHAKE);
}

static int check_handshake_ack_not_padded(struct helper *h)
{
    if (!TEST_ptr(h->qrx_pkt))
        return 0;

    return TEST_size_t_lt(h->qrx_pkt->datagram_len, h->qtx_args.mdpl);
}

static int check_handshake_ack_no_padding_frames(struct helper *h)
{
...
/* 19. Handshake ACK without padding */
static const struct script_op script_19[] = {
    OP_PROVIDE_SECRET(QUIC_ENC_LEVEL_HANDSHAKE, QRL_SUITE_AES128GCM, secret_1)
    OP_TXP_GENERATE_NONE()
    OP_CHECK(schedule_handshake_ack)
    OP_TXP_GENERATE()
    OP_RX_PKT()
    OP_CHECK(check_handshake_ack_packet)
    OP_NEXT_FRAME()
    OP_EXPECT_FRAME(OSSL_QUIC_FRAME_TYPE_ACK_WITHOUT_ECN)
    OP_CHECK(check_handshake_ack_not_padded)
    OP_CHECK(check_handshake_ack_no_padding_frames)
    OP_EXPECT_NO_FRAME()
    OP_RX_PKT_NONE()
    OP_TXP_GENERATE_NONE()
    OP_END
...
static int run_script_ex(int script_idx, const struct script_op *script)
{
...
{
    return run_script_ex(script_idx, script);
}

static int test_script(int idx)
{
    return run_script_ex(idx, scripts[idx]);
}
