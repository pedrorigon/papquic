...
For laboratory or benchmark scenarios where the server's anti-amplification
limit must be disabled, the flag
B<SSL_LISTENER_FLAG_UNLIMITED_AMPLIFICATION> may be supplied. When set, new
incoming QUIC connections are treated as already validated and can transmit
unlimited data before receiving client handshake bytes. This flag should only
be used in trusted environments because it removes an important denial-of-service
protection mandated by RFC 9000.
