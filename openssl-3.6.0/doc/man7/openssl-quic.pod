...
Applications that need to observe the detailed QUIC control flow for debugging
or benchmarking purposes may call B<OSSL_QUIC_set_debug_mode(3)> or
B<OSSL_QUIC_set_packet_log_mode(3)> with a non-zero value. When debug logging
is enabled, libssl emits timestamped diagnostic messages for key transport
events (address validation, RETRY, credit changes, and so on). Packet logging
outputs one line per transmitted packet that includes the packet type, number
and payload length (plus remaining anti-amplification credit on servers). Both
features are disabled by default to avoid any performance impact and should be
toggled at runtime before any QUIC objects are created.
