...
OUT_DIR_REL = "../outputs/heatmaps/handshake_latency"
...
def draw_heatmap(M: np.ndarray, S: np.ndarray, row_labels: list[str], col_labels: list[str],
    ...
    ax.set_xticklabels(col_labels, rotation=90, fontweight="bold", ha="center", fontsize=FONT_AXIS_X)
    ax.set_yticklabels(row_labels, rotation=0, fontweight="bold", fontsize=FONT_AXIS_Y)
...
def main():
    ...
    suffix = "_en" if args.english else "_pt"
    out_png = out_dir / f"{OUT_BASE_NAME}{suffix}.png"
    out_pdf = out_dir / f"{OUT_BASE_NAME}{suffix}.pdf"
