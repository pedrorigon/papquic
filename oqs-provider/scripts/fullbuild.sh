...
export DOQS_ALGS_ENABLED="-DOQS_ENABLE_KEM_HQC=ON"
 ...
 if [ ! -f ".local/lib/liboqs.$STATLIBEXT" ]; then
  ...
  if [ ! -d liboqs ]; then
    ...
    if [ "$LIBOQS_BRANCH" == "main" ]; then
      ...
      if [ -f oqs-template/generate.yml ]; then
        ...
        #cp oqs-template/generate.yml oqs-template/generate.yml
  ...
  cd liboqs && cmake -GNinja $CMAKE_PARAMS $DOQS_ALGS_ENABLED $CMAKE_OPENSSL_LOCATION $DOQS_LIBJADE_BUILD -DOQS_ENABLE_KEM_HQC=ON -DCMAKE_INSTALL_PREFIX=$(pwd)/../.local -S . -B _build && cd _build && ninja && ninja install && cd ../..
  if [ $? -ne 0 ]; then
